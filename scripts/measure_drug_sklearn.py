...
csv_handler = CSVHandler('sklearn_drug_with_json_hdf5.csv')
...
for i in range(10):
    ...
    sleep()
    save_csv(df, f'df_sklearn_drug_{i}.csv')
    sleep()

    # Load and save JSON
    df = load_json('../Datasets/drugs.json')
    sleep()
    save_json(df, f'df_sklearn_drug_{i}.json')
    sleep()

    # Load and save HDF5
    df = load_hdf('../Datasets/drugs.h5')
    sleep()
    save_hdf(df, f'df_sklearn_drug_{i}.h5', key='drug_data')
    sleep()
