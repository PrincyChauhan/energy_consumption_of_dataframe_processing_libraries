...
csv_handler = CSVHandler('happiness_numpy_1.24.4.csv')
...
def calculate_sum(array):
    ...
    return np.sum(numeric_data, axis=0)  # Sum along columns
...
def calculate_mean(array):
    ...
    numeric_data = np.array(numeric_data).T
    return np.mean(numeric_data, axis=0)
...
def calculate_min(array):
    ...
    numeric_data = np.array(numeric_data).T
    return np.min(numeric_data, axis=0)
...
def calculate_max(array):
    ...
    numeric_data = np.array(numeric_data).T
    return np.max(numeric_data, axis=0)

print("Starting happiness Process...")
for i in range(10):
    # Input-output functions
    array = load_csv(path='../datasets/happiness.csv')
    sleep()
    save_csv(array, f'df_numpy_happiness_{i}.csv')
    sleep()

    # Load and save JSON
    df = load_json('../datasets/happiness.json')
    sleep()
    save_json(df, f'df_numpy_happiness_{i}.json')
    sleep()

    # Load and save HDF5
    df = load_hdf('../datasets/happiness.h5')
    sleep()
    save_hdf(df, f'df_numpy_happiness_{i}.h5', key='happiness_data')
    sleep()
