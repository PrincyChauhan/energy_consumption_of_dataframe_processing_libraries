...
print("Starting HCV Data Vaex Process...")
for i in range(10):
    ...
    sleep()
    df = load_hdf(path='../datasets/hcvdata.hdf5')
    sleep()
    
    save_csv(df, f'df_hcvdata_vaex_{i}.csv')
    sleep()
    save_json(df, f'df_hcvdata_vaex_{i}.json')
    sleep()
    save_hdf(df, f'df_hcvdata_vaex_{i}.hdf5')
    sleep()

    # --------------------------------------------------
    # Handling missing data
    df = ve.read_csv('../datasets/hcvdata.csv')
    sleep()
    isna(df, cname='ALB')
    sleep()
    dropna(df)
    ...
    sleep()
    replace(df, cname='Category', src='?', dest='Unknown Category')
    sleep()

    # --------------------------------------------------
    # Table operations
    df_samp = ve.read_csv('../datasets/hcvdata.csv')
    sleep()
    drop(df, col_names=['ALB', 'ALP'])
    sleep()
    groupby(df, cname='Sex')
    sleep()
