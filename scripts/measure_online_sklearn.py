...
csv_handler = CSVHandler('onlineretail_sklearn_1.5.1.csv')
...
for i in range(10):
    ...
    sleep()
    save_csv(df, f'df_sklearn_onlineretail_{i}.csv')
    sleep()

    # Load and save JSON
    df = load_json('../datasets/onlineretail.json')
    sleep()
    save_json(df, f'df_sklearn_onlineretail_{i}.json')
    sleep()

    # Load and save HDF5
    df = load_hdf('../datasets/onlineretail.h5')
    sleep()
    save_hdf(df, f'df_sklearn_onlineretail_{i}.h5', key='onlineretail_data')
    sleep()
